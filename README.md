...
В поле уже вставлен пример 1 из методички, переведённый на Python; кнопка «Программа из
отчёта» загружает `examples/warehouse.py` — программу со всеми основными операторами языка
и пользовательскими функциями, которая анализируется в отчёте. Кнопка «Рассчитать»
...
| `examples/warehouse.py` | анализируемая программа из отчёта (склад: классы, генератор, декоратор, `match`) |
| `tools/verify_ast.py` | второй подсчёт по тем же правилам, но по дереву AST — для перекрёстной проверки |
